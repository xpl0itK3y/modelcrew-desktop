...
if (isTauri) {
  ...
  void listen<AgentHookEvent>("agent-event", (event) => {
    ...
    // приложения и в загруженный список попасть не успел. Записываем его на
    // имя из события, а не на имя панели: хук принадлежит тому конфигу, где
    // мы его прописали, и это как раз то имя. Имя панели здесь было бы
    // ошибкой с последствиями шире самого события: список общий на всё
    // приложение, и одна неверная запись заглушила бы звонки у всех панелей
    // агента, у которого хука нет и не будет.
    noteHookChannel(event.payload.agent, alert.kind);
...
// Пользователь вернулся в окно. Отметка «агент ждёт» значит «эта панель тебя
// зовёт, и ты ею ещё не занялся», а возврат в окно — это ещё не «занялся»:
// сигнал приходит только когда окно не в фокусе, так что снятие со всех
// видимых панелей разом гасило бы отметки раньше, чем их успеют увидеть, и
// вопрос «какая из панелей позвала» оставался бы без ответа. Снимаем с той
// одной, в которой пользователь действительно находится, — у xterm фокус
// держит его собственная textarea внутри контейнера панели.
