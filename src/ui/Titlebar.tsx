...
export function Titlebar(props: TitlebarProps) {
  ...
  // Счётчик на колокольчике: ровно то, что перечислено под ним в списке.
  // Ждущие агенты входят в счёт — они первыми же строками стоят в центре
  // уведомлений, и колокольчик без цифры оставлял бы их незамеченными до
  // случайного клика. Объявление гаснет, как только его прочитали, а скачанное
  // обновление остаётся в счёте до самой установки: прочитать его нельзя — по
  // нему нужно действие, и напоминание не должно исчезать от одного взгляда.
  const pendingCount =
    agentAttention +
  ...
  // Бейдж на иконке приложения повторяет колокольчик. Ноль убирает бейдж.
  const dockBadge = pendingCount;
  useEffect(() => {
    if (!isTauri) {
      return;
    }
