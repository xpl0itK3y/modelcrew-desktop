...
pub(crate) fn is_safe_repo_path(path: &str) -> bool {
    !path.is_empty()
        ...
        && !path.starts_with('-')
        && !path.contains('\\')
        // Ни одна часть пути не должна быть навигационной или пустой. `..`
        // очевиден, а вот `.` и пустая часть — нет: `.` разворачивается в сам
        // каталог, и «удалить .» означало бы снести проект целиком, а пустая
        // часть приходит из `//` и хвостового слэша.
        && !path
            .split('/')
